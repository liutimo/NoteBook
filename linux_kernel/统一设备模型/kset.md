...
1. `kset_init`
    ...
    `kset`是一个特殊的`kobject`，所以首先调用`kobject_init_internal`来初始化`k->kobj`。随后当然就是初始化链表和自旋锁咯。
...
2. `kset_register`
    ...
    初始化kset，并将其加入到hierarchy。与之对应的就是`kset_unregister`，其内部仅仅简单的调用`kobject_put(&k->kobj);`当引用计数为0时，就会自动从sysfs中移除。
